...
def load_and_check():
    ...
    data = pd.read_csv('data/sales.csv', encoding='utf-8', low_memory=False)  
    
    # FIXED: corrected the expected_column value
    expected_columns = 17
    actual_columns = data.shape[1]
    if actual_columns != expected_columns:
        print(f"Data column mismatch! Expected {actual_columns}, but got {expected_columns}.")
    ...
    # FIXED: added tax column for rows that are missing values on data['Tax']
